 ...
 * @returns {Object} Si la verificación es correcta returna verdadero en el elemento 'success' y falso en caso contrario.
 */
ExhibitionService.verifyUpdate = ({ name, lowercaseName, description, images, sponsorName, sponsorLogo, educationArea, minimumAge, maximumAge, duration, capacity, curiousInfo, questions, sound }) => {
  let serviceResponse = {
    success: true,
    content: {}
  }

  if (!name && !lowercaseName && !description && !images && !sponsorName && !sponsorLogo && !educationArea && !minimumAge && !maximumAge && !duration && !capacity && !curiousInfo && !questions && !sound) {
    serviceResponse = {
      success: false,
      content: {
 ...
 */
ExhibitionService.create = async ({ name, description, images, sponsorName, sponsorLogo, educationArea, minimumAge, maximumAge, duration, capacity, curiousInfo, questions, sound }) => {
  let serviceResponse = {
    success: true,
    content: {}
  }
